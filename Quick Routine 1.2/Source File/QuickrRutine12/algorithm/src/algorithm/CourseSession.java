...
public class CourseSession extends MaxFlow {
    int nTheoryCource;
    int nSession;
    ...
    public CourseSession(Course c[], int nCourse) {
        this.c = c;
    ...
    void setFrees(int graphCourseIndex, int graphSessionIndex, boolean value) {
        ...
        /*
        int i = Session.getFirstPeriodTimeStamp(ts);
        int j = Session.getLastPeriodTimeStamp(ts);

        for(ts = i ; ts <= j; ts++)
            dayFree[ graphCourseIndex ][ts]  = value;
        */
    ...
    void initArray() {
        nTheoryCource = Course.getCountTheoryCourse(c, nCourse);
